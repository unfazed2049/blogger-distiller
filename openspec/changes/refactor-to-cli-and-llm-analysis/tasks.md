...
- [x] 1.1 Create clis/media-crawler directory structure
...
- [x] 4.5 Implement endpoint fallback logic (reuse Python router patterns)
...
- [x] 8.1 Create scripts/utils/llm_analyzer.py module
- [x] 8.2 Design consolidated prompt template for all analysis dimensions
- [x] 8.3 Add few-shot examples for content classification
- [x] 8.4 Add few-shot examples for opinion extraction
- [x] 8.5 Add few-shot examples for writing structure analysis
- [x] 8.6 Add few-shot examples for value word extraction
- [x] 8.7 Define JSON schema for structured LLM output

## 9. LLM Analysis - API Integration

- [x] 9.1 Implement OpenAI API client with configurable endpoint
- [x] 9.2 Add support for OPENAI_API_KEY and OPENAI_API_BASE env vars
- [x] 9.3 Implement structured output mode (response_format: json_object)
- [x] 9.4 Add retry logic for API failures
- [x] 9.5 Implement token usage tracking and logging
- [x] 9.6 Add model configuration (default: gpt-4o-mini)

## 10. LLM Analysis - Output Processing

- [x] 10.1 Implement JSON schema validation for LLM output
- [x] 10.2 Create transformer to convert LLM output to legacy format
- [x] 10.3 Handle partial responses (missing sections)
- [x] 10.4 Add confidence score processing for classifications
- [x] 10.5 Implement fallback to keyword-based analysis on validation failure

## 11. Refactor analyze.py

- [x] 11.1 Replace classify_content with LLM-based classification
- [x] 11.2 Replace extract_opinion_sentences with LLM extraction
- [x] 11.3 Replace analyze_writing_structure with LLM analysis
- [x] 11.4 Replace extract_value_words with LLM extraction
- [x] 11.5 Keep keyword-based functions as fallback implementations
- [x] 11.6 Add USE_LLM_ANALYSIS environment variable flag
- [x] 11.7 Maintain exact same output JSON structure
...
- [ ] 18.1 Benchmark media-crawler CLI vs Python script performance
- [ ] 18.2 Optimize LLM prompt length to reduce token costs
- [ ] 18.3 Add caching for LLM analysis results
- [ ] 18.4 Profile memory usage for large datasets
- [ ] 18.5 Add progress indicators for long-running operations
...
- [ ] 20.1 Run full test suite and verify all tests pass
- [ ] 20.2 Test with multiple bloggers across different domains
- [ ] 20.3 Verify HTML report generation works with new data
- [ ] 20.4 Check data quality metrics (completeness, accuracy)
- [ ] 20.5 Validate cost estimates match actual usage
- [ ] 20.6 Get user feedback on analysis quality improvement
